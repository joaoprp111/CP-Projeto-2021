...
A seguir apresenta-se uma restrição ao functor base de \ensuremath{\Conid{ExpAr}}. 
Ao elemento \ensuremath{\Conid{N}\;\Varid{a}} deste data type, vamos aplicar uma função \textbf{g} ao executar o processo recursivo da estrutura.
...
Às expressões (\ensuremath{\Conid{ExpAr}}) associadas a cada operador, vamos aplicar a mesma função (diferente da função \ensuremath{\Varid{g}} porque aqui estamos a trabalhar com expressões)
 para as transformar, e essa será a função \textbf{f}.
Deste modo obtemos um novo functor base em função de \textbf{g} e \textbf{f}.
O functor \ensuremath{\Varid{recExpAr}}, por sua vez, aplica uma única função às expressões associadas aos operadores binários ou
